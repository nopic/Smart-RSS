...
(function(root, factory) {
	...
	Backbone.LocalStorage = window.Store = function(name) {
		...
		this.name = name;
		var request = window.indexedDB.open('backbone-indexeddb', 1);
		var that = this;

		request.onerror = function(event) {
			// user probably disallowed idb
			throw 'Error code: ' + event.target.errorCode;
		...
		request.onupgradeneeded = function(event) {
			// id should be replaced with thing inside idAttribute
	...
	_.extend(Backbone.LocalStorage.prototype, {
		...
		find: function(model, cb) {
			...
			request.onerror = function() {
				throw 'IndexDB Error: Can read from or write to database';
			}
	...
	Backbone.LocalStorage.sync = window.Store.sync = Backbone.localSync = function(method, model, options) {
		...
		var errorMessage, syncDfd = Backbone.$.Deferred && Backbone.$.Deferred(); //If $ is having Deferred - use it.
		options.syncDfd = syncDfd;

		var cbh = callbackHandler.bind(options);
		...
		try {
			...
			switch (method) {
				case 'read':
					...
					break;
				case 'create':
					store.create(model, cbh);
					break;
				case 'update':
					store.update(model, cbh);
					break;
				case 'delete':
					store.destroy(model, cbh);
					break;
			}
			...
			callbackHandler(null, errorMessage);
		}
